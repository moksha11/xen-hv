...
int xc_hetero_domain_increase_reservation(xc_interface *xch,
                                   uint32_t domid,
                                   unsigned long nr_extents,
                                   unsigned int extent_order,
                                   unsigned int mem_flags,
                                         ...
                                         xen_pfn_t *extent_start)
{
    ...
    printf("In xc_domain_increase_reservation_exact \n");

    err = xc_domain_increase_reservation(xch, domid, nr_extents,
                                         extent_order, mem_flags, extent_start);

    if ( err == nr_extents ) {
	    printf("Failed nr_extents %lu %u\n", nr_extents, domid);
        return 0;
	}

    if ( err >= 0 )
    {
	    printf("failed allocation %u\n", err);
      ...
      printf("Succeeded allocating pages for the guest \n");
	}

    return err;
}

int xc_hetero_domain_increase_reservation_exact(xc_interface *xch,
                                         uint32_t domid,
                                         unsigned long nr_extents,
                                         unsigned int extent_order,
                                         unsigned int mem_flags,
...
int xc_domain_hetero_incr_reserv(xc_interface *xch, unsigned int  domid, unsigned int pages) {
    
	//unsigned long nr_extents = ((unsigned long)(mem_kb)) >> (PAGE_SHIFT - 10);
	unsigned long nr_extents = pages;
    int retval = -1;

	DPRINTF("Before calling increase reservation \n");
	retval = xc_domain_increase_reservation_exact(xch, domid, nr_extents, 0, 0, NULL);
	if (retval){
	  DPRINTF("failed increase reservation \n");
	}
	return retval;
 }

int xc_alloc_heteropg(xc_interface *xch,
                       uint32_t domid,
                       int pages)
{
    int ret = -1;
    ...
    printf("Before calling increase reservation \n");
    hypercall.op     = __HYPERVISOR_alloc_hetero;
    hypercall.arg[0] = pages;

    //ret = do_xen_hypercall(xch, &hypercall);

    ret = xc_domain_hetero_incr_reserv(xch, domid, 1);
