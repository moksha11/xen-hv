...
#ifdef ENABLE_PGD
	// clean vcpu before domain_relinquish_resources() are called so that del_pgd() can delete pgd
	struct vcpu *v;
//	rcu_read_lock(&domlist_read_lock);	// I don't need this
	for_each_vcpu(d, v) {
		if (v->current_pgd) {	// lock?
			struct page_dir *pgd;
			pgd = v->current_pgd;
			v->current_pgd = NULL;
			myprintk("d%dv%d destroyed.. pgd reset.. \n", v->domain->domain_id, v->vcpu_id);
			atomic_dec(&pgd->refcnt);
		}
		v->current_kstack = NULL;
		if (v->dest_cache != -1) {
			myprintk("INFO v->dest_cache==%d ??\n");	// should be fine..
		}
		// other fields are ignored and dumped.
	}
//	rcu_read_unlock(&domlist_read_lock);
#endif
...
long vm_assist(struct domain *p, unsigned int cmd, unsigned int type)
{
	...
	printk("ENABLE %dth bit in vm_assist(0:4gb,1:4gb_noti,2:writable pt,3:pae_ext_cr3)\n", type);
        return 0;
    case VMASST_CMD_disable:
        clear_bit(type, &p->vm_assist);
	printk("DISABLE %dth bit in vm_assist(0:4gb,1:4gb_noti,2:writable pt,3:pae_ext_cr3)\n", type);
        return 0;
    }
