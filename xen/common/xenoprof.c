...
void myprof_op_init(void)
{
    ...
    xenoprof_arch_init(&xenoprof_init.num_events, xenoprof_init.cpu_type);
    myprintk("num_events:0x%x, is_primary:0x%x cpu_type:'%s'\n", xenoprof_init.num_events, xenoprof_init.is_primary, xenoprof_init.cpu_type);	// is_prim seems to be garbage value
    xenoprof_primary_profiler = current->domain;
    myprintk("set xenoprof_primary_profiler == current domain %d\n", xenoprof_primary_profiler->domain_id);
}
#endif
...
int do_xenoprof_op(int op, XEN_GUEST_HANDLE(void) arg)
{
    ...
    if ( ret )
        return ret;
#ifdef ENABLE_MYPROF
	myprintk("op:0x%x, arg:0x%x received\n", op, arg);
#endif
