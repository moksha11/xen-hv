    ...
    struct domain *d)
{
	...
	if (mini_activated)
		printk("Fail due to large order%d > %d(MAX) \n", order, MAX_ORDER);
        return NULL;
	}
    spin_lock(&heap_lock);
    /*
    ...
    if ( opt_tmem && ((order == 0) || (order >= 9)) &&
		...
		if(d && d->domain_id >= 1)	
		printk( "page_alloc: opt_tmem is set, total_avail_pages <= midsize_alloc_zone_pages \n");
        goto try_tmem;
	}
 ...
 not_found:
    ...
    spin_unlock(&heap_lock);
	if (mini_activated) {
		printk("Fail due to no suitable memory blocks\n");
...
#if 1
static void _mykeyhandler1(unsigned char key)
{
#ifdef VERBOSE_PAGE_FAULT
	pf_count = 100;
#endif
//	ptouch_print = 100; TODO
}
static struct keyhandler mykeyhandler1 = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler1,
    .desc = "page touch verbose"
};
static __init int mykeyhandler1_init(void)
{
    register_keyhandler('1', &mykeyhandler1);
    return 0;
}
__initcall(mykeyhandler1_init);

static void _mykeyhandler2(unsigned char key)
{
	if (mini_activated) {
		usched_print = 100;
	} else
...
static struct keyhandler mykeyhandler2 = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler2,
    .desc = "usched verbose"
};
static __init int mykeyhandler2_init(void)
{
    register_keyhandler('2', &mykeyhandler2);
    return 0;
}
__initcall(mykeyhandler2_init);

#ifdef ENABLE_SYSCALL_USCHED
static void _mykeyhandler3(unsigned char key)
{
	syscall_usched = 1-syscall_usched;
	myprintk("syscall_usched=%d\n", syscall_usched);
}
static struct keyhandler mykeyhandler3 = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler3,
    .desc = "syscall_usched toggle"
};
static __init int mykeyhandler3_init(void)
{
    register_keyhandler('3', &mykeyhandler3);
    return 0;
}
__initcall(mykeyhandler3_init);
#endif
#endif

#ifdef ENABLE_MYPROF
static void _mykeyhandler4(unsigned char key)
{
	int ret;
	if (mini_activated) {
void myprof_op_init(void);
		myprof_op_init();
		nmi_reserve_counters();
		myprof_counter();
		nmi_setup_events();	// basically,  XENOPROF_setup_events
		nmi_enable_virq();	// basically,  XENOPROF_enable_virq
	} else
		myprintk("Not activated\n");
}
static struct keyhandler mykeyhandler4 = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler4,
    .desc = "init myprof"
};
static __init int mykeyhandler4_init(void)
{
    register_keyhandler('4', &mykeyhandler4);
    return 0;
}
__initcall(mykeyhandler4_init);



static void _mykeyhandler5(unsigned char key)
{
	static int myprof_running = 0;
	int ret;
	if (mini_activated) {
		if (myprof_running) {
			myprintk("Stopping..\n");
			nmi_stop();	// basically, XENOPROF_stop
		} else {
			myprintk("Starting..\n");
			nmi_start();	//  basically, this is XENOPROF_start call
		}
		myprof_running = 1-myprof_running;
	} else
		myprintk("Not activated\n");
}
static struct keyhandler mykeyhandler5 = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler5,
    .desc = "myprof start/end"
};
static __init int mykeyhandler5_init(void)
{
    register_keyhandler('5', &mykeyhandler5);
    return 0;
}
__initcall(mykeyhandler5_init);
#endif


#if 1
void memleak_report(void);
static void _mykeyhandler_space(unsigned char key)
{
	myprint_xenheap();
	print_config();
	print_pgds(0);
...
#endif
	if (!mini_activated) {
		printk("Not activated\n");
/*
extern int print_dynamic_laxity;
		print_dynamic_laxity = 1-print_dynamic_laxity;
*/
		return;
	}
//	myprintk("%d pgd_migration\n", pgd_migration_count);
//	print_cache(0);
//	print_each_vcpu();
	return;
}
static struct keyhandler mykeyhandler_space = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler_space,
    .desc = "print info"
};
static __init int mykeyhandler_space_init(void)
{
    register_keyhandler(' ', &mykeyhandler_space);
    return 0;
}
__initcall(mykeyhandler_space_init);
#endif
#if 1
static void _mykeyhandler9(unsigned char key)
{
	int ret;
	if (!mini_activated)
...
static struct keyhandler mykeyhandler9 = {
    .diagnostic = 0,
    .u.fn = _mykeyhandler9,
    .desc = "Activate/deactivate"
};
static __init int mykeyhandler9_init(void)
{
    register_keyhandler('9', &mykeyhandler9);
    return 0;
}
__initcall(mykeyhandler9_init);
#endif
