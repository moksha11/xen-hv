...
#endif
#ifdef DEBUG_STAT
	memset(v->vcount, 0, sizeof(v->vcount));
#endif
#ifdef ENABLE_TIMESTAMP
	init_timestamp(&v->timestamp);
#endif
#endif
...
static void vcpu_migrate(struct vcpu *v)
{
    ...
    {
#if 1
	MYASSERT(v->dest_cache == -1);	// usched must not enter this block..
#endif
        if ( old_lock != new_lock )
            spin_unlock(new_lock);
        spin_unlock_irqrestore(old_lock, flags);
	...
	if (v->dest_cache != -1) {
		MYASSERT(new_cpu != old_cpu);
		// cpu_pick might have been called multiple times...
#ifdef VERBOSE_USCHED_DETAIL
		if (usched_print-- > 0) {
		myprintk("cpu_pick returned cpu %d\n", new_cpu);
//		if (new_cpu == 3)
//			myprintk("yes..3 is returned\n");
		}
#endif
		MYASSERT(v->run_count == 0 && v->run_cache == -1);
		v->run_count = 1;
		v->run_cache = v->dest_cache;
		v->dest_cache = -1;
	}
#endif
...
#ifdef HETERO_VISOR
int hetero_vcpu_migrate(struct vcpu *v, cpumask_t *affinity)
{
	/*if ( cpus_empty(affinity) || cpu_isset(v->processor, v->cpu_affinity)) */
	/*if ( cpus_empty(*affinity) || cpumask_equal( affinity, &(v->cpu_affinity))) */
	WARN_ON(cpus_empty(*affinity)); 
	if(cpumask_equal( affinity, &(v->cpu_affinity)))
		printk("vcpu migration to current mask\n");
...
#ifdef ENABLE_PAGE_TOUCH
// TODO: fails if sync_console is used...?
void mymigrate(cpumask_t dest_cpumask) {
	struct vcpu *v = current;
	if ( v->domain->is_pinned ) {
		mypanic("TODO: what case is this?");
	}
	MYASSERT(!cpu_isset(current->processor, dest_cpumask));
	// from __vcpu_set_affinity()
	MYASSERT( !cpus_empty(dest_cpumask) );
    vcpu_schedule_lock_irq(v);
    set_bit(_VPF_migrating, &v->pause_flags);
    vcpu_schedule_unlock_irq(v);
	// * cpu_pick function considers our dest_cache value in vcpu structure for uschedule.
	// * sync_console broke this uschedule..
        vcpu_sleep_nosync(v);	// this call --> vcpu_sleep_nosync -> csched_vcpu_sleep -> context_switch() --> context_saved() --> vcpu_migrate() --> csched_cpu_pick function...
//        vcpu_migrate(v);
...
#ifdef VERBOSE_USCHED_DETAIL
//	if (prev->dest_cache != -1)
//		myprintk("context_saved\n");
#endif
    prev->is_running = 0;
