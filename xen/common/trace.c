...
#ifdef ENABLE_MYXTRACE
	if (extra_word >= TRACE_EXTRA_MAX) {
		ASSERT((rec->u.cycles.cycles_hi >> 16) ==0);
		rec->u.cycles.cycles_hi |= (extra_word << 16);
	}
#endif
...
#ifdef ENABLE_MYXTRACE
	ASSERT(extra % sizeof(u32) == 0);
	ASSERT(extra_word < TRACE_EXTRA_MAX );	// MAX used for myxtrace
#else
