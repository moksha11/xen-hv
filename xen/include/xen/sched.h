...
#define async_exception_state(t) async_exception_state[(t)-1]
	...
	// my fields
	unsigned long myflag;	// arch/x86/x86_64/asm-offsets.c --> arch/x86/asm-offsets.s --> include/asm-x86/asm-offsets.h
#ifdef ENABLE_PGD
       struct page_dir *current_pgd;
       void *current_kstack;
#endif
	unsigned long last_eip;	// eip of last page touch
	int same_eip_count;	// # of consecutive page touches from same eip
       int dest_cache; // dest_cache for uschedule
       int run_count;	// load balancer do not steal me if non-zero run_count
//	s_time_t run_time;
       int run_cache;	// should run on this cache.
       int print_countdown;
#ifdef DEBUG_STAT
       int vcount[PTOUCH_MAX];
#endif
#ifdef ENABLE_TIMESTAMP
       struct timestamp_t timestamp;
#endif
#ifdef ENABLE_GLOBALIZE2
	struct usched_info *ui;
	struct usched_info ui_body[MAX_USCHED_INFO];
	int ui_index;
#endif
};
...
#ifdef ENABLE_PGD
struct page_dir *kernel_pgd;	// use 'current->domain->kernel_pgd' only when we have guest context.. i.e. avoid idle domain!
spinlock_t kernel_pgd_lock;
#endif
#ifdef ENABLE_GUEST_REGION
struct region_descriptor grt[MAX_RD];
spinlock_t grt_lock;
#endif
};
