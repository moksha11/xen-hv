...
static void __context_switch(void)
{
	...
	if (pgd) {
		unsigned long mfn = virt_to_mfn(pgd->pt->shadow[cache_now]);
#ifdef ENABLE_REGIONING2
		struct vregion_t *vr = pgd->current_region;
		int cpu = pgd->regioning_cpu;
		if (vr && cpu == smp_processor_id()) {	// I think this cannot race with regioning_start() function.. so safe read
			mfn = virt_to_mfn(pgd->pt->regioning_shadow);
		}
#endif
		MYASSERT(is_xen_heap_mfn(mfn));
#ifdef NO_PGD_ABOVE_4GB
		MYASSERT(!((mfn << PAGE_SHIFT) & 0xffffffff00000fff));
#endif
		_write_cr3(mfn << PAGE_SHIFT, 1);
	} else {
		_write_cr3(n->arch.cr3, 1);
	}
#else
#ifdef ENABLE_PGD
	_write_cr3(n->arch.cr3, 1);
