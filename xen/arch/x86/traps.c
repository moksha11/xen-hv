...
if (mini_activated) {
	// TODO: mini_count
	unsigned long pgdmfn = (read_cr3() >> PAGE_SHIFT);
	struct page_dir *pgd;
	
	pgd = find_pgd(pgdmfn);
	if (pgd) {
		myprintk("trapnr:%x\n", trapnr);
		do_clock(CLOCK_EVENT_TRAP, pgd, NOW());
	}
//	if (trapnr == 0x80) printk("Syscall %d ", regs->eax);
}
*/
}
...
asmlinkage void do_page_fault(struct cpu_user_regs *regs)
{
...
#ifdef VERBOSE_PAGE_FAULT
		if (pf_count>0) {
			printk("(%3d,v%d@%d):addr %p err:%x, eip:%p, esp:%p ", pf_count, current->vcpu_id, current->processor /*get_comm(regs->esp)*/ , addr, regs->error_code, regs->eip, regs->esp);
		}
#endif
	...
	// for now just skip them... TODO
	if (addr >= 0xffffffffff600000UL && addr < 0xffffffffff601000UL) {	// skip vsyscall
//		printk("[vsyscall fault??]");
#ifdef VERBOSE_PAGE_FAULT
		if (pf_count>0)
			printk(" vsyscall.");
#endif
 ...
 twobyte_opcode:
...
#if 1
		printk("WARN calling to new_guest_cr3() from arch/x86/traps.c emulation!\n");
#endif
            if ( !is_pv_32on64_vcpu(v) )
    ...
    case 0x30: /* WRMSR */ {
        ...
        case MSR_IA32_THERM_CONTROL:
            wrmsr_safe(regs->ecx, msr_content);
            break;
