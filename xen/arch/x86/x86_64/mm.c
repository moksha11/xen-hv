...
{
#if defined(ENABLE_PGD)||defined(ENABLE_INTROSPECT)
    unsigned long old_esp;
    old_esp = current->arch.guest_context.kernel_sp;
#endif
...
//		regioning_pause();//TODO.. take care of tasks within address space??
#endif
		if (current->current_kstack) {
			MYASSERT(current->current_kstack == old_esp);
		}
		current->current_kstack = esp;
		atomic_dec(&mini_count);
	}
#endif
    return 0;
