...
#if 1
	int ret = 0;
	pcount[COUNT_TOGGLE_MODE]++;
	if (v->myflag) {
	    if (v->arch.flags & TF_kernel_mode) {
		MYASSERT(v->myflag == 1);
		pcount[COUNT_SYSCALL]++;
//MYASSERT(v->arch.guest_context.user_regs.rax < 350);
#ifdef ENABLE_SYSCALL_USCHED
		if (syscall_usched /*v->current_pgd*/ /*&& v->arch.guest_context.user_regs.rax == 61*/ ) {
//			do_sched_op(0 /* SCHEDOP_yield */, 0);
			ret = uschedule(0);
		}
#endif
	    } else {
		...
		pcount[COUNT_SYSRET]++;
#ifdef ENABLE_SYSCALL_USCHED
		if (syscall_usched /*v->current_pgd*/ ) {
//			do_sched_op(0 /* SCHEDOP_yield */, 0);
			ret = uschedule(1);
		}
#endif
...
#ifdef ENABLE_SYSCALL_USCHED
	if (ret)	// if usched'ed , then skip changing cr3..
		return;	// this optimizes a bit..
#endif
	...
	print_cr3(v->arch.cr3, 3);
#endif
	unsigned long shadow_cr3 = v->arch.cr3;
#ifdef ENABLE_PER_CACHE_PT	// light-weight cache switch
	struct page_dir *pgd = NULL;
	if (pgd = v->current_pgd) {
    ...
    asm volatile ( "mov %0, %%cr3" : : "r" (shadow_cr3) : "memory" );

#else
#error assume user_mappings_are_global
#endif
#else
#ifdef USER_MAPPINGS_ARE_GLOBAL
    /* Don't flush user global mappings from the TLB. Don't tick TLB clock. */
...
#if 0	// this is needed for only 32-on-64 bit
        /*
