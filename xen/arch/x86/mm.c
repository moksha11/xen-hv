...
#ifdef ENABLE_PGD
void free_my_table(unsigned long pfn, int level)
{
       if (mini_activated) {
               MYASSERT(level == CONFIG_PAGING_LEVELS);
               atomic_inc(&mini_count);
               atomic_inc(&mini_place[0]);
               del_pgd(pfn);
               atomic_dec(&mini_place[0]);
...
static inline int update_intpte(intpte_t *p, 
                                ...
                                unsigned long mfn,
                                struct vcpu *v,
                                int preserve_ad, int my, int level)
{
    int rv = 1;
#ifdef ENABLE_PT_MODIFICATION
#ifdef DEBUG_WARN
	if (mini_activated && (!my) ) {
		myprintk("MISSED update_intpte()! %x->%x, mfn:%x\n", old,new, mfn);
		mypanic("MISSED_UPDATE");
	}
#ifdef DEBUG_ASSERT
...
#endif
#ifdef VERBOSE_PAGE_TABLE_UPDATE_ENTRY
	if (mini_activated)
		myprintk("UE:%x[%d] my=%d %x->%x", mfn, ((unsigned long)p&0xfff)>>2, my, old, new);
#endif
#endif
#ifdef ENABLE_PT_MODIFICATION
	lock_pt(mfn, level, 1);
	/*rv = paging_write_guest_entry(v, p, new, _mfn(mfn)); ---> can be just "*p=new;"*/
#endif
...
#ifdef ENABLE_PT_MODIFICATION
#define MY_UPDATE_ENTRY(_t,_p,_o,_n,_m,_v,_ad,my,level)                            \
    update_intpte(&_t ## e_get_intpte(*(_p)),                       \
                  _t ## e_get_intpte(_o), _t ## e_get_intpte(_n),   \
                  (_m), (_v), (_ad),my,level)

#define l1e_update_success_grant(_pl1e, _ol1e, _nl1e)	\
	l1e_update_success_common(_pl1e, _ol1e, _nl1e, 1)

#define l1e_update_success(_pl1e, _ol1e, _nl1e)	\
	l1e_update_success_common(_pl1e, _ol1e, _nl1e, 0)
#endif
                        ...
                        struct vcpu *pt_vcpu, struct domain *pg_dom)
{
    ...
    if ( l1e_get_flags(nl1e) & _PAGE_PRESENT )
    {
...
#ifdef ENABLE_PT_MODIFICATION
            	rc = MY_UPDATE_ENTRY(l1, pl1e, ol1e, nl1e, gl1mfn, pt_vcpu, preserve_ad, 1, 1);
		if (rc)
			goto success_l1;
#else
        ...
        adjust_guest_l1e(nl1e, pt_dom);
#ifdef ENABLE_PT_MODIFICATION
        if ( unlikely(!MY_UPDATE_ENTRY(l1, pl1e, ol1e, nl1e, gl1mfn, pt_vcpu,
                                    preserve_ad, 2, 1)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
    else if ( unlikely(!MY_UPDATE_ENTRY(l1, pl1e, ol1e, nl1e, gl1mfn, pt_vcpu,
                                     preserve_ad, 3, 1)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
success_l1:
	l1e_update_success(pl1e, ol1e, nl1e);
#endif
    return rc;
}
                        ...
                        struct vcpu *vcpu)
{
    ...
    if ( l2e_get_flags(nl2e) & _PAGE_PRESENT )
    {
...
#ifdef ENABLE_PT_MODIFICATION
            rc = MY_UPDATE_ENTRY(l2, pl2e, ol2e, nl2e, pfn, vcpu, preserve_ad, 4, 2);
		if (rc) goto success_l2;
#else
        ...
        adjust_guest_l2e(nl2e, d);
#ifdef ENABLE_PT_MODIFICATION
        if ( unlikely(!MY_UPDATE_ENTRY(l2, pl2e, ol2e, nl2e, pfn, vcpu,
                                    preserve_ad, 5, 2)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
    else if ( unlikely(!MY_UPDATE_ENTRY(l2, pl2e, ol2e, nl2e, pfn, vcpu,
                                     preserve_ad, 6, 2)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
success_l2:
	l2e_update_success(pl2e, ol2e, nl2e, pfn);
#endif
    return rc;
}
                        ...
                        struct vcpu *vcpu)
{
    ...
    if ( l3e_get_flags(nl3e) & _PAGE_PRESENT )
    {
...
#ifdef ENABLE_PT_MODIFICATION
            rc = MY_UPDATE_ENTRY(l3, pl3e, ol3e, nl3e, pfn, vcpu, preserve_ad, 7, 3);
		if (rc) {
			rc = 0;	// 0==success
			goto success_l3;
		}
        ...
        adjust_guest_l3e(nl3e, d);
#ifdef ENABLE_PT_MODIFICATION
        if ( unlikely(!MY_UPDATE_ENTRY(l3, pl3e, ol3e, nl3e, pfn, vcpu,
                                    preserve_ad, 8, 3)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
    else if ( unlikely(!MY_UPDATE_ENTRY(l3, pl3e, ol3e, nl3e, pfn, vcpu,
                                     preserve_ad, 9, 3)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
success_l3:
	l3e_update_success(pl3e, ol3e, nl3e, pfn);
#endif
    return rc;
}
                        ...
                        struct vcpu *vcpu)
{
    ...
    if ( l4e_get_flags(nl4e) & _PAGE_PRESENT )
    {
...
#ifdef ENABLE_PT_MODIFICATION
            rc = MY_UPDATE_ENTRY(l4, pl4e, ol4e, nl4e, pfn, vcpu, preserve_ad, 10, 4);
		if (rc) {
			rc = 0;	// 0==success
			goto success_l4;
		}
        ...
        adjust_guest_l4e(nl4e, d);
#ifdef ENABLE_PT_MODIFICATION
        if ( unlikely(!MY_UPDATE_ENTRY(l4, pl4e, ol4e, nl4e, pfn, vcpu,
                                    preserve_ad, 11, 4)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
    else if ( unlikely(!MY_UPDATE_ENTRY(l4, pl4e, ol4e, nl4e, pfn, vcpu,
                                     preserve_ad, 12, 4)) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
success_l4:
	l4e_update_success(pl4e, ol4e, nl4e, pfn);
#endif
    return rc;
}
...
int new_guest_cr3(unsigned long mfn)
{
    ...
    {
#if 1
	mypanic("new_guest_cr3: not-supported 32on64");
#endif
	...
	if (mini_activated)
		shadow_cr3 = change_cr3(old_base_mfn);
	if (shadow_cr3) {
#ifdef ENABLE_PER_CACHE_PT
#ifdef NO_PGD_ABOVE_4GB
		MYASSERT(!(shadow_cr3 & 0xffffffff00000fff));
#endif
		_write_cr3(shadow_cr3, 2);
#endif
	} else {
		_write_cr3(curr->arch.cr3, 2);
	}
#else
    write_ptbase(curr);
#endif
...
#ifdef VERBOSE_PINNING
// let's skip pinning, since pgd is captured during cr3 change, we better do it lazily
if (mini_activated)
	myprintk("mfn:%x pinned\n", mfn);
#endif
...
if (mini_activated) {
	mfn_check(mfn);
#ifdef VERBOSE_PINNING
	myprintk("mfn:%x unpinned\n",mfn );
#endif
	// put_page_and_type() above seems to call free_l[2,3,4]_table(), which
	// calls del_pgd(). So, maybe I don't need this here...
//	struct page_dir *pgd = find_pgd(mfn);
//	if (pgd)
//		del_pgd(pgd);
}
#endif
...
#ifdef ENABLE_PGD
	if (mini_activated) {	// mfn is new one and equal to curr->arch.guest_table_user.pfn
		struct page_dir *pgd = current->current_pgd;
//		myprintk("new_user_baseptr:%lx\n", mfn);	// occurs after every change_cr3()
		if (pgd && mfn) {	// this part is find_or_add_pgd_user()
			struct page_table *pt;
#ifdef ENABLE_PTMAN
			ptman_lock(mfn);
			MYASSERT( !find_pgd(mfn, &pt) );
			ptman_unlock(mfn);
			if (pgd->mfn_user) {
				MYASSERT(pt && pt->user_l4);
				MYASSERT(pgd->mfn_user == mfn);
			} else {
				MYASSERT(!pt);
				pt = add_pgd_user(mfn);		// TODO: make sure we handle racing...
				pgd->mfn_user = mfn;		// occurs one time for each add_pgd() (I assume)
			}
#else
			myspin_lock(&pgd_list_lock, 175);
			MYASSERT( !find_pgd_simple(mfn) );
...
#ifdef VERBOSE_NOT_A_PT
		if (mini_activated) {
			atomic_inc(&mini_count);
			atomic_inc(&mini_place[1]);
			struct page_table *pt;
			myprintk("not_a_pt: _mfn(mfn:%x):%x, val->%x\n", mfn,  _mfn(mfn), req.val );
			pt = find_pt_mfn(_mfn(mfn));
			if (pt)
				myprintk("FOUND!\n");
			atomic_dec(&mini_place[1]);
			atomic_dec(&mini_count);
		}
#endif
...
/* grant UPDATE_ENTRY()
(1) replace it with MY_UPDATE_ENTRY
(2) call l1e_update.. also note that MY_UPDATE_ENTRY() use lock_pt() !!! so make sure call unlock_pt() for all paths
*/
...
#ifdef ENABLE_PT_MODIFICATION
    if ( !MY_UPDATE_ENTRY(l1, (l1_pgentry_t *)va, ol1e, nl1e, mfn, v, 0, 13, 1) )
#else
...
#ifdef ENABLE_PT_MODIFICATION
	l1e_update_success_grant((l1_pgentry_t *)va, ol1e, nl1e);
#endif
 failed:
    ...
    uint64_t addr, unsigned long frame, struct domain *d)
{
    ...
    page_unlock(page);
#ifdef ENABLE_PT_MODIFICATION
	l1e_update_success_grant((l1_pgentry_t *)va, ol1e, l1e_empty());
#endif
 failed:
    unmap_domain_page(va);
    put_page(page);
    ...
    unsigned long va, l1_pgentry_t nl1e, struct vcpu *v)
{
    ...
    ol1e = *pl1e;
#ifdef ENABLE_PT_MODIFICATION
    okay = MY_UPDATE_ENTRY(l1, pl1e, ol1e, nl1e, gl1mfn, v, 0, 15, 1);
#else
...
#ifdef ENABLE_PT_MODIFICATION
	if (okay)
		l1e_update_success_grant(pl1e, ol1e, nl1e);
#endif
    guest_unmap_l1e(v, pl1e);
    ...
    unsigned long addr, unsigned long frame, l1_pgentry_t nl1e, struct vcpu *v)
{
    ...
    /* Delete pagetable entry. */
#ifdef ENABLE_PT_MODIFICATION
    if ( unlikely(!MY_UPDATE_ENTRY(l1, pl1e, ol1e, nl1e, gl1mfn, v, 0, 16, 1)) )
#else
    ...
    uint64_t addr, unsigned long frame, uint64_t new_addr, unsigned int flags)
{
...
#ifdef ENABLE_PT_MODIFICATION
    if ( unlikely(!MY_UPDATE_ENTRY(l1, pl1e, ol1e, l1e_empty(),
                                gl1mfn, curr, 0, 17, 1)) )
#else
    ...
    put_page(l1pg);
#ifdef ENABLE_PT_MODIFICATION
	l1e_update_success_grant(pl1e, ol1e, l1e_empty());
#endif
    guest_unmap_l1e(curr, pl1e);
...
#ifdef DEBUG_WARN
unsigned long find_l1e_mfn(unsigned long addr)
{
    unsigned long mfn;
#if CONFIG_PAGING_LEVELS == 4
...
//	if (mini_activated)
//		myprintk("(va:%x:xx(%x)=%x)\n", gl1mfn, ((unsigned long)pl1e&0xfff), *pl1e);

//static inline unsigned long gmfn_to_mfn(struct domain *d, unsigned long gpfn)
#ifdef DEBUG_WARN
#ifndef ENABLE_PER_CACHE_PT
	if (mini_activated && gl1mfn!=find_l1e_mfn(va) ) {
		myprintk("WARN!! gl1mfn:%x , 2mfn:%x , ret=%x\n", gl1mfn, gmfn_to_mfn(current->domain, gl1mfn) , find_l1e_mfn(va) );
	}
#endif
#endif
    ...
    struct ptwr_emulate_ctxt *ptwr_ctxt)
{
...
#ifdef ENABLE_PT_MODIFICATION
		myprintk("INFO last path I missed..\n");
		unlock_pt(0);	// it failed, so just pass
#endif
            return X86EMUL_CMPXCHG_FAILED;
        }
#ifdef ENABLE_PT_MODIFICATION
	// if success, it comes here and will reach l1e_update_success below
#endif
    }
    else
    {
        ol1e = *pl1e;
#ifdef ENABLE_PT_MODIFICATION
        if ( !MY_UPDATE_ENTRY(l1, pl1e, ol1e, nl1e, mfn, v, 0, 101, 1) )	// this includes lock_pt()
#else
	...
	if (mini_activated) {
		atomic_inc(&mini_count);
		atomic_inc(&mini_place[2]);
#ifdef VERBOSE_PAGE_TABLE_PTWR_EMULATED_UPDATE
		myprintk("PTWR: %x[%d] %x->%x, pl1e:%x\n", mfn, ((unsigned long)pl1e&0xfff)>>2, old, nl1e.l1, pl1e);
#endif
		l1e_update_success(pl1e, ol1e, nl1e);	// this includes unlock_pt()
		atomic_dec(&mini_place[2]);
		atomic_dec(&mini_count);
	} else if (this_cpu(found_pt)) {
		unlock_pt(0);	// in case of disabling...
	}
#endif
    unmap_domain_page(pl1e);
