...
/*HETEROMEMFIX: ALL HETEROMEM definitions to be moved to
* configuration files
*/
#define HETEROMEM
...
static void increase_reservation(struct memop_args *a)
{
    ...
    //printk("Entering increase_reservation \n");
    ...
    //printk("Entering multipage_allocation_permitted\n");
    if ( !multipage_allocation_permitted(current->domain, a->extent_order) )
        return;

    //printk("Entering loop for trying allocation\n");
    for ( i = a->nr_done; i < a->nr_extents; i++ )
    {
       ...
       //printk("Allocation domheap pages \n");
        ...
        //printk("Allocation is successful\n");
...
static int increase_reservation_pgs(struct memop_args *a)
{
    struct page_info *page;
    unsigned long i;
    xen_pfn_t mfn;
...
//#ifdef HETEROMEM	
			// If heteromem is defined the check which node
			//printk(KERN_DEBUG "HETEROMEM node %d \n", XENMEMF_get_node(a->memflags);
            page = alloc_domheap_pages(d, a->extent_order, a->memflags);
//#else
			/*if(d->domain_id > 0)*/
			page = alloc_domheap_pages(d, a->extent_order, (a->memflags | MEMF_node(SLOW_MEMORY_NODE)));
//#endif
...
long do_memory_op(unsigned long cmd, XEN_GUEST_HANDLE(void) arg)
{
	...
	/*HeteroMem Changes*/
	/* We start reading memflags to guest domanin hints*/
        ...
        {
        case XENMEM_increase_reservation:
	    //printk("memop: increase_reservation called \n");	
//#ifndef HETEROMEM
            increase_reservation(&args);
//#else
//			rc = increase_reservation_pgs(&args);
//			goto exitout;
//#endif
            break;
        case XENMEM_decrease_reservation:
